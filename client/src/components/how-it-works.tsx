      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:block relative">
          ...
          <div className="grid grid-cols-4 gap-6">
            {steps.map((step: any, i: number) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.15 }} className="relative" data-testid={`step-${i}`}>
                  <div className="text-center">
                    <div className="relative mx-auto w-16 h-16 rounded-full bg-obsidian border-2 border-arctic/30 flex items-center justify-center mb-6">
                      ...
                      <span className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-xpertblue text-pure text-xs font-bold flex items-center justify-center font-mono">{stepNumbers[i]}</span>
                    </div>
        ...
        <div className="md:hidden space-y-6">
          {steps.map((step: any, i: number) => {
            const Icon = stepIcons[i];
            return (
              <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }} className="flex gap-5" data-testid={`step-mobile-${i}`}>
                <div className="flex-shrink-0 relative">
                  ...
                  <span className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-xpertblue text-pure text-[10px] font-bold flex items-center justify-center font-mono">{stepNumbers[i]}</span>
                  {i < steps.length - 1 && <div className="absolute top-16 left-1/2 w-px h-8 bg-arctic/15 -translate-x-1/2" />}
                </div>
                <div className="pt-1">
                  <h3 className="font-heading font-semibold text-pure text-base mb-1">{step.title}</h3>
                  <span className="text-arctic text-xs font-medium">{step.subtitle}</span>
